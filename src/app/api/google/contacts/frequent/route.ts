...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${GOOGLE_SERVICE_URL}/contacts/frequent`);
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Failed to fetch frequent contacts' }));
    ...
    console.error('Frequent contacts error:', error);
