...
export async function GET() {
  try {
    ...
    // Call Google Service to get authorization URL
    const response = await fetch(`${GOOGLE_SERVICE_URL}/oauth/authorize`, {
      headers: {
    ...
    if (!response.ok) {
      await response.text();
      return NextResponse.json({ error: 'Failed to get authorization URL' }, { status: response.status });
    }
